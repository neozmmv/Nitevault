...
export const fetchFiles = async (): Promise<FileItem[] | undefined> => {
    try {
        const res = await fetch(`${API_URL}/api/storage/list`, { credentials: "include" });
        if (!res.ok) {
...
export async function deleteFile(fileId: string) {
    try {
        const res = await fetch(`${API_URL}/api/storage/file/${fileId}`, { method: "DELETE", credentials: "include"})
        return res.ok;
    } catch (err) {
...
export async function getDownloadToken(fileId: string): Promise<string | undefined> {
    try {
        const res = await fetch(`${API_URL}/api/storage/generateToken/${fileId}`, {
            credentials: "include"
        });
